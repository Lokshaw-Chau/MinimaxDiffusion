...
python train.py -d imagenet --imagenet_dir ./results/dit-${guide_type}/${spec}-${nsample}-${cp}-${ntype}-k${k}-f${f}-gamma${gamma}-r${r}-gi${gi}-low${low}-high${high} /root/share/ImageNet \
        -n resnet --depth 18 --nclass 10 --norm_type instance --ipc 50 --tag rgd_woof_ipc50_f15 --slct_type random --spec woof --repeat 3

python train.py -d imagenet --imagenet_dir ./results/dit-${guide_type}/${spec}-${nsample}-${cp}-${ntype}-k${k}-f${f}-gamma${gamma}-r${r}-gi${gi}-low${low}-high${high} /root/share/ImageNet \
        -n resnet --depth 18 --nclass 10 --norm_type instance --ipc 100 --tag rgd_woof_ipc100_f15 --slct_type random --spec woof --repeat 1