...
for f in 1 5 10 15; do
        ...
        python train.py -d imagenet --imagenet_dir ./results/dit-${guide_type}-dino/${spec}-${nsample}-${cp}-${ntype}-k${k}-f${f}-gamma${gamma}-r${r}-gi${gi}-low${low}-high${high} /root/share/ImageNet \
                -n resnet --depth 18 --nclass 10 --norm_type instance --ipc 10 --tag rgd_dino_woof_ipc10_f${f} --slct_type random --spec woof --repeat 3
        
        python train.py -d imagenet --imagenet_dir ./results/dit-${guide_type}-dino/${spec}-${nsample}-${cp}-${ntype}-k${k}-f${f}-gamma${gamma}-r${r}-gi${gi}-low${low}-high${high} /root/share/ImageNet \
                -n resnet --depth 18 --nclass 10 --norm_type instance --ipc 50 --tag rgd_dino_woof_ipc50_f${f} --slct_type random --spec woof --repeat 3

        python train.py -d imagenet --imagenet_dir ./results/dit-${guide_type}-dino/${spec}-${nsample}-${cp}-${ntype}-k${k}-f${f}-gamma${gamma}-r${r}-gi${gi}-low${low}-high${high} /root/share/ImageNet \
                -n resnet --depth 18 --nclass 10 --norm_type instance --ipc 100 --tag rgd_dino_woof_ipc100_f${f} --slct_type random --spec woof --repeat 1
