...
def define_model(args, nclass, logger=None, size=None):
    ...
    if args.net_type == 'resnet':
        ...
        model = CN.ConvNet(channel=args.nch, num_classes=nclass, net_width=128, net_depth=3, net_act='relu', net_norm='instance', net_pooling='avgpooling', im_size=(args.size, args.size))
...
def main(args, logger, repeat=1):
    ...
    for i in range(repeat):
        ...
        train(args, model, train_loader, val_loader, plotter, logger)
...
def train(args, model, train_loader, val_loader, plotter=None, logger=None):
    ...
    state = torch.load('/root/workspace/MinimaxDiffusion/ckpts/woof/convnet6/replay_buffer_0.pt')
    ckpts = state[0]
    ckpt = ckpts[-1]
    ckpt = torch.cat([p.data.to('cuda').reshape(-1) for p in ckpt], 0).requires_grad_(True)
    model = ReparamModule(model)

    model = model.cuda()

    acc1, acc5, loss_val = validate(args, val_loader, model, criterion, 0, logger, ckpt)
    print(f'Initial accuracy (top-1 and 5): {acc1:.1f} {acc5:.1f}')
...
def validate(args, val_loader, model, criterion, epoch, logger=None, ckpt=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (input, target) in enumerate(val_loader):
        input = input.cuda()
        target = target.cuda()
        output = model(input, flat_param=ckpt)
