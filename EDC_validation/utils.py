...
def load_model(model_name="resnet18", dataset="cifar10", pretrained=True, classes=[]):
    def get_model(model_name="resnet18"):
        if "conv" in model_name:
            ...
            model = torchvision.models.__dict__["resnet101"](pretrained=False)
            model.conv1 = nn.Conv2d(
                3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False
            )
            model.maxpool = nn.Identity()
    ...
    if pretrained:
        if dataset in [
            ...
            checkpoint = torch.load(
                f"/root/workspace/MinimaxDiffusion/squeeze/squeeze_wo_ema/woof/resnet18/{dataset}_{model_name}.pth", map_location="cpu"
            )
